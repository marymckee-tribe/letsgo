  ...
  return (
    <main className="flex-1 w-full bg-white text-black flex flex-col p-12 lg:p-24 overflow-hidden h-[calc(100vh-6rem)]">
      <div className="mx-auto max-w-[1600px] w-full h-full flex flex-col">
        ...
        <div className="flex justify-between items-end mb-12 shrink-0 border-b border-black/10 pb-6">
          <div>
            <h1 className="font-heading text-6xl font-light tracking-tighter text-black mb-2">Activity Stream</h1>
            <p className="text-black/60 text-sm uppercase tracking-widest font-mono">System-wide Immutable Operations Log</p>
          </div>
          <div className="hidden sm:flex items-center gap-4 text-xs tracking-widest uppercase font-bold border border-black/10 text-black px-4 py-2 bg-black/5">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto pr-8">
           <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-black/20 text-xs uppercase tracking-widest font-bold text-black/40">
              ...
              <tbody>
                {logs.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="text-black/40 italic font-serif py-8">Awaiting initial system logs...</td>
                  </tr>
                ) : logs.map((log) => {
                  const date = new Date(log.timestamp)
                  return (
                    <tr key={log.id} className="border-b border-black/5 hover:bg-black/[0.02] transition-colors group">
                      <td className="py-4 text-xs font-mono tabular-nums text-black/60 align-top">
                      ...
                      <td className="py-4 align-top">
                        <span className="text-[10px] uppercase font-mono bg-black/5 px-2 py-1 text-black/40 group-hover:text-black/60 transition-colors">
                          {log.id.slice(0, 12)}
                        </span>
                      </td>
                      <td className="py-4 align-top">
                        <span className="font-serif italic text-black/90">"{log.message}"</span>
                      </td>
                    </tr>
                  )
                })}
